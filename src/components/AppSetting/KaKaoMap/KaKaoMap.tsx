...
export default function KaKaoMap({mapToggleBtn,userEditing}:settingProps) {
  const { kakao } = window;
  const [inputText, setInputText] = useState<string>("");
  const [searchData, setSearchData] = useState<string>("");
  const ctx = useWeatherContext();
  const { setUserAddress, setUserLoaction } = ctx
  ...
  useEffect(() => {
    ...
    function displayPlaces(places:any) {
      ...
      for (var i = 0; i < places.length; i++) {
        ...
        (function (marker, title,user_address,user_position) {
      ...
      <div className="map_wrap">
        ...
        <div id="menu_wrap" className="bg_white">
          <div className="option">
            <div>
              <form >
                키워드 : <input type="text" value={inputText} onChange={(e) => { setInputText(e.target.value) }} id="keyword" size={15} />
                <button
                  onClick={(e) => {
                    e.preventDefault();
