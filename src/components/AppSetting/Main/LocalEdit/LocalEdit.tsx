...
import type { settingProps } from '../Main';

export default function LocalEdit({ mapToggleBtn, userEditing,editing }:settingProps) {
  const ctx = useWeatherContext();
  const { userAddress,userAddressList, setUserAddressList,userLocation } = ctx
  const [editingId, setEditingId] = useState<number>(0);

  useEffect(() => {
    const today:Date = new Date();
    const id:number = today.getTime();   // 시간정보로 아이디 값을 생성

    const check = userAddressList.find((aa) => userAddress === aa.address )   // 중복인지 체크
    if (!(check) && userAddress) {    // 중복이 아니고 초기상태도 아니라면
      if (editing) {  // 편집 중일 경우
        const search:AddressList | undefined = userAddressList.find((aa) => aa.id === editingId);   // 편집 중인 요소를 검색
        let copy:AddressList[] = [...userAddressList];    // 아마 참조복사가 되는듯. 생각한 결과는 아니지만 문제는 없음
        if(userLocation) {
          copy.forEach((aa) => {
            if(search && aa.id === search.id) {
              aa.Ma = userLocation.Ma;
              aa.La = userLocation.La;
              aa.address = String(userAddress) ; // 편집 중인 요소의 주소와 좌표 정보를 변경
            }
          })
        ...
        if(userLocation) {
          const newList:AddressList = {   // 선택한 주소의 정보를 담는 객체를 생성
            id,
  ...
  function editBtn(key:number) {   // 편집 버튼을 누르면 맵을 켜고 편집하려는 요소의 id를 저장함
    mapToggleBtn();
    userEditing(true);
    setEditingId(key);
  }
  function removeBtn(key:number) {   // 삭제 버튼을 누르면 선택한 id를 userAddressList에서 검색해서 삭제
    const searchList = userAddressList.find((aa) => aa.id === key )
    if(searchList && searchList.selected) alert("북마크한 목록은 삭제할 수 없습니다.") 
